 ...
 */
public class GetFollowingHandler extends PagedServerHandler<User> implements RequestHandler<PagedRequest<User>, PagedResponse> {
     ...
     */
    @Override
    public PagedResponse handleRequest(PagedRequest<User> request, Context context) {
        ...
        PagedResponse<User> response = service.getFollowing(request);
        System.out.println("GetFollowingHandler.handleRequest returning response with followers: " + response.getItems());
        return response;
    }
